...
#Check for samples to process in metadata
metadata_file = config["metadata"]
metadata = pd.read_csv(metadata_file, sep="\t")
metadata.columns = metadata.columns.str.strip()

if "SampleID" not in metadata.columns:
    raise ValueError(f"'SampleID' column not found. Available columns: {metadata.columns.tolist()}")

valid_samples = set(metadata["SampleID"].dropna().tolist())
...
def listFastq(wildcards):
    ...
    if os.path.isdir(sample_path):
        ...
        subdirs = [
            ...
            if os.path.isdir(os.path.join(sample_path, d)) and d in valid_samples
        ]
        ...
        if subdirs:
            ...
            for file in files:
                ...
                if wildcards.sample in file_base and file_base in valid_samples:
...
def validate_boolean_config(config_value, config_name):
    if not isinstance(config_value, bool):
        raise ValueError(
            f"Invalid value for {config_name}: '{config_value}'. It must be True or False (case-sensitive)."
        )
...
def get_samples():
    ...
    # Load metadata and extract valid sample IDs
    metadata_file = config["metadata"]
    metadata = pd.read_csv(metadata_file, sep="\t")
    metadata.columns = metadata.columns.str.strip()  # Remove hidden spaces
    valid_samples = set(metadata["SampleID"])  # Only process these samples
    ...
    if os.path.isdir(sample_path):
        ...
        if subdirs:
            ...
            for file in files:
                ...
                if file_base in valid_samples and file_base not in samples:
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        sys.exit(
            "Usage: python merge_abund_tax_blast.py <tax_file> <abund_file> <output_file>"
        )
...
def prepare_inputs():
        ...
        inputs.extend(
            expand(
                os.path.join(
                    ...
                    "final",
                    "{id}",
                    "phyloseq_abundance_{id}_sintax.tsv",
                ),
                id=ids,
            )
        )
        ...
        inputs.extend(
            expand(
                os.path.join(
                    ...
                    "R-environment",
                    "{id}",
                    "R-environment_{id}_sintax.RData",
                ),
                id=ids,
            )
        )
    if config["include_blast_output"]:
        ...
        inputs.extend(
            expand(
                os.path.join(
                    config["output_dir"], "final", "figs", "{id}", "Ordination_{id}_blast.png"
                ),
                id=ids,
            )
        )
        inputs.extend(
            expand(
                os.path.join(
                    config["output_dir"], "final", "figs", "{id}", "Boxplot_{id}_blast.png"
                ),
                id=ids,
            )
        )
        inputs.extend(
            expand(
                os.path.join(
                    config["output_dir"], "final", "R-environment", "{id}", "R-environment_{id}_blast.RData"
                ),
                id=ids,
            )
        )
    if not any((config["include_sintax_output"], config["include_blast_output"])):
        inputs.extend(
            expand(
                os.path.join(config["output_dir"], "final", "{id}", "OTU_table_{id}.tsv"),
                id=ids,
            )
        )
        inputs.extend(
            expand(
                os.path.join(config["output_dir"], "final", "{id}", "OTUs_{id}.fna"),
                id=ids,
            )
        )
